...
export async function getTotalLikes(postId) {
  const likes = await Event.countDocuments({ post: postId, action: 'like' })
  const unlikes = await Event.countDocuments({ post: postId, action: 'unlike' })
  return {
    likes: Math.max(0, likes - unlikes),
  }
}
